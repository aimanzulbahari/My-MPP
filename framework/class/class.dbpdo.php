...
class Db {
    ...
    public static function conn_db($dbdata) {
        ...
        //echo "$dbtype:host=$host;dbname=$database; $user, $password";
        try {
    ...
    public static function sendmail($from, $to, $title, $content, $cc = '', $bcc='', $attachment = '') {
        $mail = new PHPMailer();
        ...
        if (!$mail->send()) {
            //alert('Mail delivery is not successful');
            //echo "Mailer Error (" . str_replace("@", "&#64;", $to['emel']) . ') ' . $mail->ErrorInfo . '<br />';
        } else {
            return 1;
        }
        //end:send email
